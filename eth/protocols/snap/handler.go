...
func ServiceGetAccountRangeQuery(chain *core.BlockChain, req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	...
	tr, err := trie.New(req.Root, chain.StateCache().TrieDB(), 0)
	if err != nil {
		return nil, nil
	}
...
func ServiceGetStorageRangesQuery(chain *core.BlockChain, req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			accTrie, err := trie.New(req.Root, chain.StateCache().TrieDB(), 0)
			if err != nil {
				return nil, nil
			}
			...
			stTrie, err := trie.New(acc.Root, chain.StateCache().TrieDB(), 0)
			if err != nil {
				return nil, nil
			}
			proof := light.NewNodeSet()
...
func ServiceGetTrieNodesQuery(chain *core.BlockChain, req *GetTrieNodesPacket, start time.Time) ([][]byte, error) {
	...
	accTrie, err := trie.NewSecure(req.Root, triedb, 0)
	if err != nil {
	...
	for _, pathset := range req.Paths {
		...
		default:
			...
			stTrie, err := trie.NewSecure(common.BytesToHash(account.Root), triedb, 0)
			loads++ // always account database reads, even for failures
			if err != nil {
				break
			}
