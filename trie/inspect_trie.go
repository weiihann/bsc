...
func NewInspector(tr *Trie, blocknum uint64, jobnum uint64) (*Inspector, error) {
	if tr == nil {
...
func (inspect *Inspector) Run() {
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				inspect.trie.db.Cap(DEFAULT_TRIEDBCACHE_SIZE)
			}
		}
	}()
...
func (inspect *Inspector) ConcurrentTraversal(theTrie *Trie, theTrieTreeStat *TrieTreeStat, theNode node, height uint32, path []byte) {
	...
	case hashNode:
		n, err := theTrie.resolveHash(current, nil)
		if err != nil {
	...
	case valueNode:
		...
		contractTrie, err := New(account.Root, theTrie.db, 0)
		if err != nil {
