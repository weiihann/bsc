...
func testBlockChainImport(chain types.Blocks, pipelineCommit bool, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		statedb, err := state.New(blockchain.GetBlockByHash(block.ParentHash()).Root(), blockchain.stateCache, nil, block.NumberU64(), nil)
		if err != nil {
			return err
		}
...
func TestTransientStorageReset(t *testing.T) {
	...
	// Check the storage
	state, err := chain.StateAt(chain.CurrentHeader().Root, chain.CurrentBlock().Number.Uint64())
	if err != nil {
