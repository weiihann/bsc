...
func SetupGenesisBlockWithOverride(db ethdb.Database, genesis *Genesis, overrideBerlin, overrideArrowGlacier, overrideTerminalTotalDifficulty *big.Int) (*params.ChainConfig, common.Hash, error) {
	...
	if _, err := state.New(header.Root, state.NewDatabaseWithConfigAndCache(db, nil), nil, header.Number.Uint64()); err != nil {
		if genesis == nil {
			genesis = DefaultGenesisBlock()
		}
...
func (g *Genesis) ToBlock(db ethdb.Database) *types.Block {
	...
	statedb, err := state.New(common.Hash{}, state.NewDatabase(db), nil, 0)
	if err != nil {
		panic(err)
	}
