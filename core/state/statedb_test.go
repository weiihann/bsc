...
func TestUpdateLeaks(t *testing.T) {
	...
	state, _ := New(common.Hash{}, NewDatabase(db), nil, 0)
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transState, _ := New(common.Hash{}, NewDatabase(transDb), nil, 0)
	finalState, _ := New(common.Hash{}, NewDatabase(finalDb), nil, 0)
...
func TestCopy(t *testing.T) {
	...
	orig, _ := New(common.Hash{}, NewDatabase(rawdb.NewMemoryDatabase()), nil, 0)
...
func (test *snapshotTest) run() bool {
	...
	for sindex--; sindex >= 0; sindex-- {
		checkstate, _ := New(common.Hash{}, state.Database(), nil, 0)
...
func TestMissingTrieNodes(t *testing.T) {
	...
	state, _ := New(common.Hash{}, db, nil, 0)
	addr := common.BytesToAddress([]byte("so"))
	{
		state.SetBalance(addr, big.NewInt(1))
