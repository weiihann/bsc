...
type snapshot interface {
	...
	Update(blockRoot common.Hash, blockNum uint64, destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte, storage map[common.Hash]map[common.Hash][]byte, verified chan struct{}) *diffLayer
...
func (t *Tree) Update(blockRoot common.Hash, blockNum uint64, parentRoot common.Hash, destructs map[common.Address]struct{}, accounts map[common.Address][]byte, storage map[common.Address]map[string][]byte, verified chan struct{}) error {
	hashDestructs, hashAccounts, hashStorage := transformSnapData(destructs, accounts, storage)
	return t.update(blockRoot, blockNum, parentRoot, hashDestructs, hashAccounts, hashStorage, verified)
}

// Update adds a new snapshot into the tree, if that can be linked to an existing
// old parent. It is disallowed to insert a disk layer (the origin of all).
func (t *Tree) update(blockRoot common.Hash, blockNum uint64, parentRoot common.Hash, destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte, storage map[common.Hash]map[common.Hash][]byte, verified chan struct{}) error {
	...
	snap := parent.(snapshot).Update(blockRoot, blockNum, destructs, accounts, storage, verified)
