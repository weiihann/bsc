...
func TestMergeBasics(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), common.Hash{}, 0, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), nil)
	child := newDiffLayer(parent, common.Hash{}, 0, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), nil)
	child = newDiffLayer(child, common.Hash{}, 0, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), nil)
	child = newDiffLayer(child, common.Hash{}, 0, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), nil)
	child = newDiffLayer(child, common.Hash{}, 0, copyDestructs(destructs), copyAccounts(accounts), copyStorage(storage), nil)
	// And flatten
...
func TestMergeDelete(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), common.Hash{}, 0, flipDrops(), flipAccs(), storage, nil)
	child := parent.Update(common.Hash{}, 0, flopDrops(), flopAccs(), storage, nil)
	child = child.Update(common.Hash{}, 0, flipDrops(), flipAccs(), storage, nil)
	child = child.Update(common.Hash{}, 0, flopDrops(), flopAccs(), storage, nil)
	child = child.Update(common.Hash{}, 0, flipDrops(), flipAccs(), storage, nil)
	child = child.Update(common.Hash{}, 0, flopDrops(), flopAccs(), storage, nil)
	child = child.Update(common.Hash{}, 0, flipDrops(), flipAccs(), storage, nil)
...
func TestInsertAndMerge(t *testing.T) {
	...
	)
	{
		...
		parent = newDiffLayer(emptyLayer(), common.Hash{}, 0, destructs, accounts, storage, nil)
	}
	{
		var (
			destructs = make(map[common.Hash]struct{})
		...
		child = newDiffLayer(parent, common.Hash{}, 0, destructs, accounts, storage, nil)
	}
	// And flatten
	merged := (child.flatten()).(*diffLayer)
...
func BenchmarkSearch(b *testing.B) {
	...
	fill := func(parent snapshot) *diffLayer {
		...
		return newDiffLayer(parent, common.Hash{}, 0, destructs, accounts, storage, nil)
	}
	var layer snapshot
	layer = emptyLayer()
	for i := 0; i < 128; i++ {
